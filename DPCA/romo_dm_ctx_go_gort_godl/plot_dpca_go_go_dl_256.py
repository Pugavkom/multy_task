...
def main():
    ...
    for n_task, task in enumerate(tasks):
        for i in range(len(components)):
            a = np.load(os.path.join("data", f"Z_a_{components[i]}_{task}_256_1_2.npy"))
            v = np.load(os.path.join("data", f"Z_v_{components[i]}_{task}_256_1_2.npy"))
            plot_graph(a, v, axes[j], axes[j + 1], letters[n_task])
        j += 2
    axes[0].legend(ncol=3, loc="lower left", bbox_to_anchor=(-0.0, 1), shadow=True)
    plt.tight_layout()
    plt.savefig("dpca_256_go_and_godl.eps")
