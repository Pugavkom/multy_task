...
def main():
    pca_a_romo = np.load("pca/256_1_2_low_freq_low_filter_2999/pca_a_go.npy")
    pca_v_romo = np.load("pca/256_1_2_low_freq_low_filter_2999/pca_v_go.npy")
    colors_romo = np.load("pca/256_1_2_low_freq_low_filter_2999/colors_pca_go.npy")
    sp_names_l = list(np.load("pca/256_1_2_low_freq_low_filter_2999/go_sp_names.npy"))
    sp_names = []
    for el in sp_names_l:
        value = str(round(float(el.split("$")[-1]), 4))
        sp_names.append(el.replace(el.split("$")[-1], value))
    del sp_names_l
    ...
    ax2.view_init(60, 45)
    plt.savefig("Pca_go.eps", format="eps")
    plt.show()
    # plt.show()
    #    plt.close()
