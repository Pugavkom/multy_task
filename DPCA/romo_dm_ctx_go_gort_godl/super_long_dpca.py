...
def generate_task_romo_dm_ctx_go_gort_godl(
    ...
    for value in go_task_list_values:
        ...
        dm_parameters = DMTaskRandomModParameters(
            dm=DMTaskParameters(trial_time=0.8, value=value)
        )
...
def main():
    ...
    model.load_state_dict(
        torch.load(
            (
                r"A:\src\multy_task\notebooks\train\reduce\lif_adex\romo_dm_ct_go_gort_godl\super_long_intervals_list_go_task_improve_correct_functions_two_trials\Train_dm_and_romo_task_reduce_lif_adex_without_refrac_random_delay_long_a_alpha_100_N_450"
            )
        )
    )

    actR_v, acrR_a = dpca_run(model, check_tasks, device, 10, hidden_size)
    ...
    for i in range(len(z_v_list)):
        for key in z_v_list[i]:
            np.save(
                f"data_long_rt{os.sep}Z_v_{key}_{sorted(tasks)[i]}", z_v_list[i][key]
            )
            np.save(
                f"data_long_rt{os.sep}Z_a_{key}_{sorted(tasks)[i]}", z_a_list[i][key]
            )
